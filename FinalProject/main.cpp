...
struct monitor_t {
  ...
  pthread_mutex_t mutex_segs = PTHREAD_MUTEX_INITIALIZER;
  pthread_mutex_t mutex_parent = PTHREAD_MUTEX_INITIALIZER;
  pthread_mutex_t mutex_child = PTHREAD_MUTEX_INITIALIZER;
  pthread_cond_t child_thread_cond[SEG_CNT];
  ...
  monitor_t() {
    for (uint8_t i = 0; i < SEG_CNT; i++) {
      child_thread_cond[i] = PTHREAD_COND_INITIALIZER;
    }
  }
};
...
void* inputParse_Thread1(void* vid_ptr) {
  ...
  while (1) {
    ...
    if(!vid->read(inFrame)) {
      ...
      for (uint8_t i = 0; i < SEG_CNT; i++) {
        pthread_cond_signal(&monitor.child_thread_cond[i]);
      }
    ...
    while (monitor.frame_handler_state[frame_head] && !monitor.eof) {
      ...
      pthread_cond_wait(&monitor.parent_thread_cond, &monitor.mutex_parent);
    }
    monitor.parent_ready = 0x1;

    pthread_mutex_unlock(&monitor.mutex_parent);
    pthread_mutex_lock(&monitor.mutex_child);
    for (uint8_t i = 0; i < SEG_CNT; i++) {
    ...
    pthread_mutex_unlock(&monitor.mutex_child);
    pthread_mutex_lock(&monitor.mutex);
...
void* processSegment_Thread2(void* seg_idx_ptr) {
  ...
  int frame_tail = 0;
  static thread_local cv::Mat seg_in_local, grayscale_local, sobel_local;
  while (1) {
    ...
    while (!monitor.eof && !monitor.frame_handler_state[frame_tail] && !monitor.parent_ready) {
      pthread_cond_wait(&monitor.child_thread_cond[idx], &monitor.mutex_parent);      
    }
    ...
    if (monitor.frame_handler_state[frame_tail]) {
      seg_in_local = monitor.frame_InUse[frame_tail].seg_in[idx];

      // Unlock Child Thread; Allow other threads to process while this thread applies filters locally
      pthread_mutex_unlock(&monitor.mutex_child);

      if (grayscale_local.empty()) {
        grayscale_local.create(seg_in_local.rows, seg_in_local.cols, CV_8UC1);
      }
      if (sobel_local.empty()) {
        sobel_local.create(seg_in_local.rows, seg_in_local.cols, CV_8UC1);
      }

      // Process Segment Mat (Apply Filters)
      to442_grayscale(seg_in_local, grayscale_local);
      to442_sobel(grayscale_local, sobel_local);
      ...
      // Lock Child Thread before exiting
      pthread_mutex_lock(&monitor.mutex_child);
    }
...
void* recombFrame_Thread3(void*) {
  ...
  while (1) {
    ...
    while ((!monitor.eof) && !(monitor.frame_handler_state[frame_tail] &&
      ...
      pthread_cond_wait(&monitor.output_thread_cond, &monitor.mutex_child);
    }
    ...
    if (monitor.frame_handler_state[frame_tail]) {
      ...
      if (outFrame.empty()) {
        outFrame.create(full_height, full_width, CV_8UC1);
      }
      for (uint8_t i = 0; i < SEG_CNT; i++) {
...
int main(int argc, char** argv) {
  ...
  } else if ((argc == 3) && (std::string(argv[2]) == "--no-gui")) {
    show_gui = 0x0;
    filename = argv[1];
    vid.open(filename);
    if (!vid.isOpened()) {
