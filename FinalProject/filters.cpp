...
void to442_grayscale(const cv::Mat &RGBFrame, cv::Mat &outFrame) {
  ...
  const uint8x16_t kR_vec = vdupq_n_u8(kR);     // Create vector copy of static variable
  const uint8x16_t kG_vec = vdupq_n_u8(kG);
  const uint8x16_t kB_vec = vdupq_n_u8(kB);
  ...
  for (int rowIDX = 0; rowIDX < RGBFrame.rows; rowIDX++) {
    ...
    // Iterate every 16 Pixels (columns) in row
    int colIDX;
    for (colIDX = 0; colIDX <= (RGBFrame.cols - 16); colIDX += 16) {
      // Obtain RGB values by grouping 16 RGB Pixels into {B, G, R} vectors
      // ie) {B0 G0 R0}, {B1 G1 R1}, {B2 G2 R2} ... {B15 G15 R15}
      //     0 Vec3b      1 Vec3b     2 Vec3b         15 Vec3b
      uint8x16x3_t BGR = vld3q_u8(inPixel_1b + (3 * colIDX));  // Iterate every 48 bytes
      uint8x16_t B = BGR.val[0];
      uint8x16_t G = BGR.val[1];
      uint8x16_t R = BGR.val[2];

      // Split to 8-Lane Halves for Widening Multiplies
      uint8x8_t Rlow = vget_low_u8(R);
      uint8x8_t Glow = vget_low_u8(G);
      uint8x8_t Blow = vget_low_u8(B);
      uint8x8_t Rhi = vget_high_u8(R);
      uint8x8_t Ghi = vget_high_u8(G);
      uint8x8_t Bhi = vget_high_u8(B);

      // Calculate Grayscale Conversion (ITU-R (BT.709))
      // Temporarily convert to 16-bit value to account for integer multiplication (Do each half: low - hi)
      uint16x8_t acclow = vmull_u8(Rlow, vget_low_u8(kR_vec));
      acclow = vmlal_u8(acclow, Glow, vget_low_u8(kG_vec));
      acclow = vmlal_u8(acclow, Blow, vget_low_u8(kB_vec));

      uint16x8_t acchi = vmull_u8(Rhi, vget_low_u8( kR_vec));
      acchi = vmlal_u8(acchi, Ghi, vget_low_u8(kG_vec));
      acchi = vmlal_u8(acchi, Bhi, vget_low_u8(kB_vec));

      // Divide by 256 to get true ITU-R Values, then reduce back to 8-bit value
      uint8x8_t grayLow = vrshrn_n_u16(acclow, INT_CONV_BITS);
      uint8x8_t grayHi = vrshrn_n_u16(acchi, INT_CONV_BITS);
      uint8x16_t gray = vcombine_u8(grayLow, grayHi);

      // Store grayscale value to Output Frame
      vst1q_u8(outPixel + colIDX, gray);
    }
